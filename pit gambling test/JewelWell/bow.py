...
def jewelBowLore(tier, lives, infoList):
  global infoCommand
  # updates list
  if tier == 1:
  ...
  if tier < 3:
    tier += 1
    jewelBowLore(tier, lives, infoList)
...
def tier0_1(infoList):
  ...
  infoList[0] = randint(0, len(bowList[infoList[6]][infoList[3]]) - 1) # randomize enchant 1 index
  return infoList
