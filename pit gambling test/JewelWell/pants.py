...
def jewelPantsLore(tier, lives, color, infoList):
  ...
  itemLore = ""
  if infoList[3] != -1:
    enchant1 = pantsList[infoList[6]][infoList[3]][infoList[0]]
    itemLore += enchant1
  if infoList[4] != -1:
    enchant2 = pantsList[infoList[7]][infoList[4]][infoList[1]]
    itemLore += "\n" + enchant2
  if infoList[5] != -1:
    enchant3 = pantsList[infoList[8]][infoList[5]][infoList[2]]
    itemLore += "\n" + enchant3
  ...
  # prints everything
  if tier == 0:
    print(Fore.YELLOW + "Held item:" + Style.RESET_ALL + "\n-------------- " + Fore.CYAN + "Hidden Jewel\n" + Style.RESET_ALL + changeTierColor(color) + itemPrefix + tiers[tier] + colors[color] + "Pants" + Style.DIM + Fore.RESET + "\nDyed\nKept on death" + Style.RESET_ALL + changeTierColor(color) + "\n\nUsed in the mystic well\nAlso, a fashion statement" + Fore.BLUE + "\nUnbreakable" + Style.RESET_ALL + "\n----------------------------")
  else:
    print(Fore.YELLOW + "Held item:" + Style.RESET_ALL + "\n-------------- " + Fore.CYAN + "Hidden Jewel\n" + Style.RESET_ALL + changeTierColor(color) + itemPrefix + tiers[tier] + colors[color] + "Pants\n" + Style.DIM + Fore.RESET + "Dyed\nLives: " + Style.RESET_ALL + Fore.GREEN + str(lives) + Style.DIM + Fore.RESET + "/" + str(lives) + Style.RESET_ALL + "\n" + itemLore + changeTierColor(color) + "\n\nAs strong as iron" + Fore.BLUE + "\nUnbreakable" + Style.RESET_ALL + "\n----------------------------")
  ...
  if tier == 3 and (input1 == "gem" or input1 == "g"):
    ...
    if gemmedEnchant != "c" and gemmedEnchant != "cancel":
      print(gem(infoList, color, gemmedEnchant, tier, lives))
  ...
  if tier < 3:
    tier += 1
    jewelPantsLore(tier, lives, color, infoList)

def changeTierColor(color):
...
def get1stEnchant(infoList):
  if infoList[6] == 3:
    ...
    color = Fore.BLUE
  if infoList[3] != -1:
    return color + pantsList[infoList[6]][infoList[3]][infoList[0]].split("\n")[1] + Style.RESET_ALL
  return "-"
def get2ndEnchant(infoList):
  if infoList[7] == 3:
    ...
    color = Fore.BLUE
  if infoList[4] != -1:
    return color + pantsList[infoList[7]][infoList[4]][infoList[1]].split("\n")[1] + Style.RESET_ALL
  return "-"
def get3rdEnchant(infoList):
  if infoList[8] == 3:
    ...
    color = Fore.BLUE
  if infoList[5] != -1:
    return color + pantsList[infoList[8]][infoList[5]][infoList[2]].split("\n")[1] + Style.RESET_ALL
  return Fore.RED + "✖" + Style.RESET_ALL

def gem(infoList, color, gemmedItem, tier, lives):
...
def tier0_1(infoList):
  ...
  infoList[3] = 2 # define enchant 1 to be 3 tokens
  infoList[6] = randint(0,2) # define enchant 1 type
  while infoList[6] == 1: # prevent resourcs enchants
    infoList[6] = randint(0,3)
  if addRare == 0:
    infoList[6] = 3 # make enchant 1 rare
  infoList[0] = randint(0, len(pantsList[infoList[6]][infoList[3]]) - 1) # randomize enchant 1 index
  return infoList
  ...
  if addRare != 0: # -===-dont add rare-===-
    for i in range(tokensAdded):
      ...
      if infoList[enchantAddedTo - 1] == -1: # if enchant hasnt been defined yet
        infoList[enchantAddedTo - 1] = randint(0, len(pantsList[infoList[enchantAddedTo + 5]][infoList[enchantAddedTo + 2]]) - 1) # define enchant
...
def fixDupeEnchants(infoList, enchantChecked):
  if enchantChecked == 1:
    while (infoList[0] == infoList[1] and infoList[6] == infoList[7]) or (infoList[0] == infoList[2] and infoList[6] == infoList[8]):
      infoList[0] = randint(0, len(pantsList[infoList[6]][infoList[3]]) - 1)
    ...
    while (infoList[1] == infoList[0] and infoList[7] == infoList[6]) or (infoList[1] == infoList[2] and infoList[7] == infoList[8]):
      infoList[1] = randint(0, len(pantsList[infoList[7]][infoList[4]]) - 1)
    ...
    while (infoList[2] == infoList[0] and infoList[8] == infoList[6]) or (infoList[2] == infoList[1] and infoList[8] == infoList[7]):
      infoList[2] = randint(0, len(pantsList[infoList[8]][infoList[5]]) - 1)
  return infoList
